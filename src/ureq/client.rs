...
impl GateHttpClient {
    ...
    pub fn send<R: Into<Request>>(&self, request: R) -> Result<Response, Box<Error>> {
        ...
        // Stringify payload (body)
        let payload_string = crate::utils::payload_to_json_str(payload.clone())
            .map_err(|_| Error::PayloadSerializationError)?;
        ...
        if let Some(Credentials { api_key, api_secret }) = request_credentials.or(client_credentials)
        {
            if sign {
                ...
                let query_string: String = ureq_request
                    .request_url()
                    .unwrap()
                    .as_url()
                    .query()
        ...
        let raw_response = if payload.is_empty() {
            ...
            ureq_request.send_string(&payload_string) // Request with body
        };
